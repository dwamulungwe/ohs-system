...
class PermitToWork(OrganisationOwnedMixin, TimestampMixin, Base):
    ...
    approved_by_user_id: Mapped[Optional[int]] = mapped_column(ForeignKey("users.id", ondelete="SET NULL"), index=True, nullable=True)
    assigned_team_or_contractor: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
    ...
    required_worker_user_ids: Mapped[list[int]] = mapped_column(MutableList.as_mutable(JSON), default=list, nullable=False)
    required_contractor_worker_ids: Mapped[list[int]] = mapped_column(MutableList.as_mutable(JSON), default=list, nullable=False)
    eligibility_enforcement_enabled: Mapped[bool] = mapped_column(default=False, nullable=False)
    eligibility_validation: Mapped[dict] = mapped_column(JSON, default=dict, nullable=False)
    ...
    issued_by: Mapped[Optional["User"]] = relationship(foreign_keys=[issued_by_user_id], lazy="selectin")
    approved_by: Mapped[Optional["User"]] = relationship(foreign_keys=[approved_by_user_id], lazy="selectin")
